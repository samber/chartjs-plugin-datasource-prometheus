...
export function fillGaps(
  ...
  step: number,
  options: ChartDatasourcePrometheusPluginOptions,
) {
  ...
  chart.data.datasets.forEach((dataSet: ChartDataset, index: number) => {
    ...
    for (let i = dataSet.data.length - 2; i > 0; i--) {
      if (dataSet.data[i + 1]["x"] - dataSet.data[i]["x"] > 1100 * minStep) {
        for (
          let steps =
            (dataSet.data[i + 1]["x"] - dataSet.data[i]["x"]) /
            (minStep * 1000);
          steps > 1;
          steps--
        ) {
    ...
    if (Math.abs(start.getTime() - dataSet.data[0]["x"]) > 1100 * minStep) {
      for (
        let i =
          Math.abs(start.getTime() - dataSet.data[0]["x"]) / (minStep * 1000);
        i > 1;
        i--
      ) {
        (chart.data.datasets[index].data as any).unshift({
    ...
    if (
      Math.abs(end.getTime() - dataSet.data[dataSet.data.length - 1]["x"]) >
      1100 * minStep
    ) {
      for (
        let i =
        ...
        (chart.data.datasets[index].data as any).push({
          x: new Date(
            dataSet.data[chart.data.datasets[index].data.length - 1][
              "x"
            ].getTime() +
              minStep * 1000,
          ),
